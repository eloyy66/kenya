...
def _distraction_always_run() -> bool:
    """Permite ejecutar distraccion en cada request (sin cache temporal)."""

    return os.getenv("MOSTACHO_DISTRACTION_ALWAYS_RUN", "0").strip().lower() in {"1", "true", "yes", "on"}


def _distraction_use_full_frame() -> bool:
    """Permite usar frame completo para distraccion."""

    return os.getenv("MOSTACHO_DISTRACTION_USE_FULL_FRAME", "0").strip().lower() in {"1", "true", "yes", "on"}


def _distraction_crop_params() -> Tuple[float, float, float]:
    """Obtiene parametros de expansion del recorte para distraccion.

    Retorna (scale_x, scale_y, center_y_offset_ratio).
    """
    ...
    scale_x = _read("MOSTACHO_DISTRACTION_CROP_SCALE_X", 1.55, 1.0, 3.0)
    scale_y = _read("MOSTACHO_DISTRACTION_CROP_SCALE_Y", 1.85, 1.0, 3.5)
    offset_y = _read("MOSTACHO_DISTRACTION_CROP_CENTER_Y_OFFSET", 0.10, -0.40, 0.80)
    return scale_x, scale_y, offset_y


def _distraction_threshold() -> float:
    """Umbral de decision para etiquetar DISTRACTED en salida binaria."""

    raw_value = os.getenv("MOSTACHO_DISTRACTION_THRESHOLD", "0.75").strip()
    try:
        value = float(raw_value)
    except ValueError:
        value = 0.75
...
def _binary_distraction_probs(class_probabilities: Dict[str, float]) -> Tuple[float, float] | None:
    """Extrae (normal_prob, distracted_prob) si existen clases binarias reconocidas."""

    lowered = {str(key).strip().lower().replace(" ", "_"): float(value) for key, value in class_probabilities.items()}
    normal_prob = lowered.get("normal")
    distracted_prob = lowered.get("distracted")
    if normal_prob is None and distracted_prob is None:
        return None
    if normal_prob is None and distracted_prob is not None:
        normal_prob = 1.0 - float(distracted_prob)
    if distracted_prob is None and normal_prob is not None:
        distracted_prob = 1.0 - float(normal_prob)
    if normal_prob is None or distracted_prob is None:
        return None
    return float(normal_prob), float(distracted_prob)
...
def _resolve_distraction(face_crop: np.ndarray, face_response: FaceResponse | None, now: float) -> Dict[str, Any]:
    ...
    should_run = (
        _distraction_always_run()
        or (_DISTRACTION_LAST_RESULT is None)
        or ((now - _DISTRACTION_LAST_RUN_AT) >= interval_seconds)
    )
    ...
    if should_run:
        ...
        if class_probabilities:
            binary_probs = _binary_distraction_probs(class_probabilities)
            if binary_probs is not None:
                normal_prob, distracted_prob = binary_probs
                threshold = _distraction_threshold()
                if distracted_prob >= threshold:
                    raw_state = "DISTRACTED"
                    raw_confidence = float(distracted_prob)
                else:
                    raw_state = "NORMAL"
                    raw_confidence = float(normal_prob)
            else:
...
def _resolve_distraction_crop(
    image: np.ndarray,
    payload: DrowsinessImageRequest,
    face_response: FaceResponse | None,
) -> np.ndarray:
    """Construye un recorte ampliado para distraccion (mas contexto que solo rostro)."""

    if _distraction_use_full_frame():
        return image

    face_bbox: Sequence[float] | None = None
    if face_response is not None and face_response.detections:
        face_bbox = face_response.detections[0].bbox
    elif payload.face_bbox:
    ...
    except Exception:  # pragma: no cover - defensa runtime
        return image
    ...
    scale_x, scale_y, offset_y = _distraction_crop_params()
    crop_w = face_w * scale_x
    crop_h = face_h * scale_y
    center_y = center_y + (face_h * offset_y)
    ...
    crop = crop_image_by_bbox(image, expanded_bbox)
    if crop.size > 0:
        return crop

    # Fallback seguro al recorte facial base.
    face_crop = _resolve_face_crop(image=image, payload=payload, face_response=face_response)
    if face_crop.size > 0:
        return face_crop
    return image
...
def predict_drowsiness_image(payload: DrowsinessImageRequest) -> DrowsinessResponse:
    ...
    distraction_crop = _resolve_distraction_crop(image, payload, face_response)

    now = time.monotonic()
    model_ran, run_reason = _should_run_drowsiness(face_response, eye_response, now)
    distraction_payload = _resolve_distraction(face_crop=distraction_crop, face_response=face_response, now=now)
